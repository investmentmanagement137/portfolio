...
function App() {
  ...
  const [activeView, setActiveView] = useState<'dashboard' | 'dividend_details'>('dashboard');
  ...
  return (
    <div className="min-h-screen bg-[#1e1e1e] text-[#e3e3e3] p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        <header className="mb-10 text-center relative border-b border-[#3c4043] pb-8">
          <h1 className="text-4xl font-bold text-white mb-2 flex items-center justify-center gap-3">
            <LayoutDashboard className="w-10 h-10 text-[#8ab4f8]" />
            Portfolio Analyzer
          </h1>
          <p className="text-[#b4b4b4]">Secure Portfolio Intelligence Engine</p>
          
          <div className="absolute right-0 top-0 flex items-center gap-3">
             {data && (
                <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium border ${isLtpLoading ? 'bg-yellow-900/20 border-yellow-700/50 text-yellow-500' : 'bg-green-900/20 border-green-700/50 text-green-500'}`}>
                   {isLtpLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />}
                   {isLtpLoading ? 'Syncing Market...' : 'Market Live'}
                </div>
             )}
            {data && (
              <button 
                onClick={handleClear}
                className="text-sm text-red-400 hover:text-red-300 hover:bg-red-900/20 px-3 py-2 rounded-lg flex items-center gap-2 transition-colors border border-transparent hover:border-red-900/50"
              >
        ...
        {!data ? (
          <div className="bg-[#2d2d2d] rounded-2xl shadow-2xl p-8 max-w-4xl mx-auto border border-[#3c4043]">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="space-y-4">
                ...
                <div 
                  className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors cursor-pointer ${historyFile ? 'border-green-400 bg-green-50' : 'border-[#3c4043] hover:border-[#8ab4f8]/50'}`}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 text-[#b4b4b4] mb-1">
                    ...
                    <span className="text-sm font-medium uppercase tracking-wider">Total Cash Dividend Received</span>
                  </div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="bg-[#1e1e1e] border border-[#3c4043] rounded-lg pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-[#8ab4f8]"
                    value={searchTerm}
                  ...
                  <tbody className="divide-y divide-[#3c4043]">
                    {sortedDetails.map((item, idx) => (
                      <tr key={idx} className="hover:bg-[#353535] transition-colors">
                        <td className="px-6 py-4 font-mono text-[#b4b4b4]">{item["Book Closure Date"]}</td>
                        <td className="px-6 py-4 font-bold text-[#8ab4f8]">{item.Scrip}</td>
                        <td className="px-6 py-4 text-right font-mono text-[#e3e3e3]">{item.Holdings}</td>
                        <td className="px-6 py-4 text-right font-mono text-[#e3e3e3]">{item["Cash %"]}%</td>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className={`p-6 rounded-2xl border ${portfolioSummary.pl >= 0 ? 'bg-green-900/10 border-green-800/50' : 'bg-red-900/10 border-red-800/50'}`}>
                ...
                <table className="w-full text-left relative">
                  <thead className="bg-[#353535] sticky top-0 z-10 text-[#b4b4b4]">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-right">Yield</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#3c4043]">
            ...
            <div className="bg-[#2d2d2d] rounded-2xl border border-[#3c4043] overflow-hidden">
              <div className="p-6 border-b border-[#3c4043] flex justify-between items-center">
                ...
                <button 
                  onClick={() => setActiveView('dividend_details')}
