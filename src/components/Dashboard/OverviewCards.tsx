...
export function OverviewCards() {
    const { state: { portfolioSummary } } = usePortfolio();
    const { investment, value, pl, plPercent, activeDividendTotal } = portfolioSummary;
    ...
    return (
        <Card className="mb-8 overflow-hidden border-none bg-gradient-to-br from-muted/50 via-card to-muted/30 shadow-2xl relative group">
            ...
            <CardContent className="p-0">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x border-border/50">
                    <div className="p-6 transition-colors hover:bg-muted/30">
                        ...
                        <div className="space-y-1">
                            <div className="text-2xl font-mono text-foreground font-bold tracking-tight">
                    ...
                    <div className="p-6 transition-colors hover:bg-muted/30">
                        ...
                        <div className="space-y-1">
                            <div className={cn("text-2xl font-mono font-bold tracking-tight flex items-baseline gap-2", isProfit ? "text-green-500" : "text-red-500")}>
                                {isProfit ? '+' : ''}{formatCurrency(pl)}
                                <span className="text-xs opacity-80 font-bold">({plPercent.toFixed(2)}%)</span>
                            </div>
